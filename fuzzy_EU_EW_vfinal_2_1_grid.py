...
class KernelFCMWEWEU2:
    ...
    # ADICIONADO: Parâmetro verbose=True para suprimir os prints no Grid Search
...
if __name__ == "__main__":
    ...
    print("--- Configuração do Dataset (KFCM-K-W-EwEu.2_1) ---")
    ...
    if modo == '2':
        ...
        # Função isolada para o Joblib
        def avaliar_parametros(Tw_test, Tu_test, n_reps, X_data, n_clusters, max_iter, tol):
            min_dists_rodadas = []
            for rep in range(n_reps):
                model_test = KernelFCMWEWEU2(n_clusters=n_clusters, Tw=Tw_test, Tu=Tu_test, tol=tol, max_iter=max_iter, random_state=rep)
    ...
    # --- Execução Principal do KFCM-K-W-EwEu.2_1 ---
    ...
    for i in range(n_repetitions_input):
        #print(f"\n--- Repetição {i+1}/{n_repetitions_input} (KFCM-K-W-EwEu.2_1) ---")
    ...
    df_G.to_csv("EwEu.2_1_prototipos_finais_otimos.csv", index_label="cluster")
    
    df_s_squared.to_csv("EwEu.2_1_s_squared_otimos.csv", index_label="cluster")
    
    pd.DataFrame(best_U).to_csv("EwEu.2_1_pertinencia_fuzzy_otima.csv", index=False)
    ...
    if y_true is not None:
        ...
        best_cm_df.to_csv("EwEu.2_1_matriz_de_confusao_otima.csv", index=True)
        
    df_results.to_csv("EwEu.2_1_resultados_clusters_otimos.csv", index=False)
    ...
    if y_true is not None and len(all_metrics_history) > 0:
        ...
        stats_df.to_csv("EwEu.2_1_metricas_estatisticas_gerais.csv", index_label="Metrica")
        print("\nEstatísticas (Média, Desvio, Mediana, Máx, Mín) salvas em 'EwEu.2_1_metricas_estatisticas_gerais.csv'")

        print(f"\n--- Métricas da Melhor Execução (Rodada {best_round}) ---")
        best_metrics = df_all_metrics[df_all_metrics['Round'] == best_round].iloc[0]
        print(best_metrics)
        
        best_metrics.to_frame().T.to_csv("EwEu.2_1_metricas_melhor_rodada.csv", index=False)
        df_all_metrics.to_csv("EwEu.2_1_historico_metricas_todas_rodadas.csv", index=False)
