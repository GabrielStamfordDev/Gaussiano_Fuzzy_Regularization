...
class KernelFCMWEWEU2:
    ...
    # Step 1: Atualiza s_squared usando Algoritmo 4 (Eq. 38 e 40)
    def update_s_squared(self, X, G, U):
        ...
        # Recupera os valores de 1/s^2 da iteração anterior (t-1) para congelamento
        prev_inv_s_squared = np.zeros((K_clusters, D))
        mask_prev = self.s_squared > 1e-15
        prev_inv_s_squared[mask_prev] = 1.0 / self.s_squared[mask_prev]
        
        # --- ALGORITMO 4: Loop sobre cada cluster i ---
        for i in range(K_clusters):
            ...
            gamma_prime = 1.0 # Inicializa gamma' (linha 2 do Algoritmo 4)
            update = True
            ...
            while update:
                ...
                for j in J_indices:
                    # Eq 38 usando gamma_prime
                    num = (gamma_prime + card_J) * np.exp(- Q_i[j] / self.Tw)
                ...
                if len(remove_list) > 0:
                    for idx_to_remove in remove_list:
                        if idx_to_remove in J_indices:
                            ...
                            # Congela no valor da iteração anterior (t-1)
                            frozen_val = prev_inv_s_squared[i, idx_to_remove]
                            new_inv_s_squared_matrix[i, idx_to_remove] = frozen_val
                            # Atualiza gamma' subtraindo o valor congelado (Eq 40)
    ...
    def fit(self, X):
        ...
        while(iteration_count <= self.max_iter):
            ...
            # Step 1: Atualização de s_squared (Algoritmo 4 / Eq. 38)
...
if __name__ == "__main__":
    ...
    for i in range(n_repetitions_input):
        ...
        print(f"\nValor final da função J_KFCM-K-W-EwEu.2: {current_objective_value:.6f}")
        ...
        if y_true is not None:
            ...
            print("\n\n--- Matriz de Confusão ---")
            print("Não foi possível gerar a Matriz de Confusão (Target não fornecido).")
    ...
    print("\n\n--- Resultado Final (Melhor Execução - KFCM-K-W-EwEu.2) ---")
    ...
    if y_true is not None:
        ...
        print("\nMatriz de Confusão ótima salva em 'EwEu.2_matriz_de_confusao_otima.csv'")
    else:
    ...
    print("Protótipos salvos em 'EwEu.2_prototipos_finais_otimos.csv'")
    
    df_s_squared.to_csv("EwEu.2_s_squared_otimos.csv", index_label="cluster")
    print("Parâmetros de Largura (s^2) salvos em 'EwEu.2_s_squared_otimos.csv'")
    
    df_U = pd.DataFrame(best_U)
    df_U.to_csv("EwEu.2_pertinencia_fuzzy_otima.csv", index=False)
    print("Matriz U salva em 'EwEu.2_pertinencia_fuzzy_otima.csv'")
    ...
    print("Dados com labels salvos em 'EwEu.2_resultados_clusters_otimos.csv'")
    ...
    if y_true is not None:
        ...
        metrics_df.to_csv("EwEu.2_metricas_avaliacao_detalhadas.csv", index=False)
        print("\nArquivo 'EwEu.2_metricas_avaliacao_detalhadas.csv' salvo com sucesso.")
...
# -------------------- MÉTRICAS FUZZY BASEADAS EM PARES --------------------
    ...
    try:
        ...
        metrics_df_final.to_csv("EwEu.2_metricas_avaliacao_fuzzy_completas.csv", index=False)
        print("\nArquivo 'EwEu.2_metricas_avaliacao_fuzzy_completas.csv' salvo com sucesso.")
