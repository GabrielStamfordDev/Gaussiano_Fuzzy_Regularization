...
class KernelFCMWEWEU1:
    ...
    def update_s_squared(self, X, G, U):
            ...
            # Recupera os valores de 1/s^2 da iteração anterior (t-1)
            prev_inv_s_squared = np.zeros(D)
            mask = self.s_squared > 1e-15
            prev_inv_s_squared[mask] = 1.0 / self.s_squared[mask]
            
            gamma_prime = 1.0 # Inicializa gamma' (linha 1 do Algoritmo 3)
            update = True
                ...
                if len(remove_list) > 0:
                    for idx_to_remove in remove_list:
                        if idx_to_remove in J_indices:
                            ...
                            # Linha 8 do Algoritmo 3: congela no valor da iteração anterior (t-1)
                            frozen_val = prev_inv_s_squared[idx_to_remove]
                            new_inv_s_squared[idx_to_remove] = frozen_val
                            # Linha 9 do Algoritmo 3: Atualiza gamma' subtraindo o valor congelado (Eq 39)
...
if __name__ == "__main__":
    ...
    for i in range(n_repetitions_input):
        ...
        print(f"Valor final da função J_KFCM-K-W-EwEu.1: {current_objective_value:.6f}")
    ...
    if y_true is not None and len(all_metrics_history) > 0:
        ...
        print("\nEstatísticas (Média, Desvio, Mediana, Máx, Mín) salvas em 'EwEu.1_metricas_estatisticas_gerais.csv'")

        print(f"\n--- Métricas da Melhor Execução (Rodada {best_round}) ---")
        best_metrics = df_all_metrics[df_all_metrics['Round'] == best_round].iloc[0]
        print(best_metrics)
        
        best_metrics.to_frame().T.to_csv("EwEu.1_metricas_melhor_rodada.csv", index=False)
