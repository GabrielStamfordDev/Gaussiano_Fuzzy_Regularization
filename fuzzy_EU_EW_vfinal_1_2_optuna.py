...
if __name__ == "__main__":
    ...
    for i in range(n_repetitions_input):
        #print(f"\n--- Repetição {i+1}/{n_repetitions_input} (KFCM-K-W-EwEu.1_2) ---")
    ...
    # --- Salvamento de Dados (Atualizado para o sufixo .1_2_) ---
    pd.DataFrame(best_G, columns=feature_names).to_csv("EwEu.1_2_prototipos_finais_otimos.csv", index_label="cluster")
    df_s_squared = pd.DataFrame(best_s_squared.reshape(1,-1), columns=feature_names)
    df_s_squared.T.rename(columns={0: 's^2'}).to_csv("EwEu.1_2_s_squared_otimos.csv", index_label="feature")
    pd.DataFrame(best_U).to_csv("EwEu.1_2_matriz_de_pertencimento_fuzzy_otima.csv", index=False)
    ...
    if y_true is not None:
        ...
        best_cm_df.to_csv("EwEu.1_2_matriz_de_confusao_otima.csv", index=True)
        
    df_results.to_csv("EwEu.1_2_resultados_com_clusters_otimos.csv", index=False)
    ...
    if y_true is not None and len(all_metrics_history) > 0:
        ...
        print(stats_df)
        stats_df.to_csv("EwEu.1_2_metricas_estatisticas_gerais.csv", index_label="Metrica")
        df_all_metrics[df_all_metrics['Round'] == best_round].to_csv("EwEu.1_2_metricas_melhor_rodada.csv", index=False)
        df_all_metrics.to_csv("EwEu.1_2_historico_metricas_todas_rodadas.csv", index=False)