...
    df_G.to_csv("prototipos_finais_kfcm-ew1_otimos.csv", index_label="cluster")
    print("Matriz de Protótipos (G) ótima salva em 'prototipos_finais_kfcm-ew1_otimos.csv'")
    
    # Salva o vetor s^2
    df_s_squared_output = df_s_squared.T.rename(columns={0: 's^2'})
    df_s_squared_output.to_csv("s_squared_kfcm-ew1_otimos.csv", index_label="feature")
    print("Vetor de Parâmetros de Largura (s^2) salvo em 's_squared_kfcm-ew1_otimos.csv'")
    
    # Salva a Matriz U (Pertinência Fuzzy)
    df_U = pd.DataFrame(best_U)
    df_U.to_csv("matriz_de_pertencimento_fuzzy_kfcm-ew1_otima.csv", index=False)
    print("Matriz de Pertinência Fuzzy (U) ótima salva em 'matriz_de_pertencimento_fuzzy_kfcm-ew1_otima.csv'")
...
        
    df_results.to_csv("resultados_com_clusters_kfcm-ew1_otimos.csv", index=False)
    print("Dados originais com os rótulos de cluster ótimos salvos em 'resultados_com_clusters_kfcm-ew1_otimos.csv'")
...
        # O ARI mede a similaridade entre duas atribuições de dados, ignorando permutações de rótulos.
        # É ideal para clustering pois não exige que o Cluster 0 seja igual à Classe 0.
        ari = adjusted_rand_score(y_true, best_labels)
        print(f"ARI (Adjusted Rand Index): {ari:.4f}")

        # 1.2 NMI (Normalized Mutual Information) - Métrica baseada em Teoria da Informação
        # Não depende do mapeamento de classes, compara as distribuições diretamente.
        nmi = normalized_mutual_info_score(y_true, best_labels)
        print(f"NMI (Normalized Mutual Information): {nmi:.4f}")

        # ----------------------------------------------------------------     
        # ENTROPIA CONDICIONAL (Métrica de Avaliação de Qualidade)
        #------------------------------------------------------------------
        # Esta é a usada para dizer se o cluster é "puro".
        # Usa dois somatórios (Clusters e Classes).
...
                
                # Entropia interna deste cluster (Impureza)
                cluster_impurity = 0
                for cls in unique_classes:
                    # Probabilidade da classe j DADO o cluster k
...
        # 3. Mapeamento de Clusters para Classes (Necessário para Acurácia e F-measure)
...
        
        # Cria um mapeamento temporário para exibir ao usuário
        cluster_class_map = {}

        for k in range(n_clusters_input):
            # Obtém os índices dos dados que caíram no cluster k
            mask = (best_labels == k)
            
            if np.sum(mask) > 0:
                # Pega os rótulos verdadeiros desses dados
                true_labels_in_cluster = y_true_np[mask]
                
                # Encontra a classe mais frequente (moda) neste cluster
                # Usamos pd.Series para facilitar a busca da moda em strings ou números
                most_frequent_class = pd.Series(true_labels_in_cluster).mode()[0]
                
                # Atribui essa classe a todos os elementos deste cluster no vetor mapeado
                predicted_labels_mapped[mask] = most_frequent_class
                cluster_class_map[k] = most_frequent_class
            else:
                # Caso raro de cluster vazio
                pass
        
        # Converte para o mesmo tipo do y_true para evitar erros de tipo no sklearn
...
        # 7. F-measure POR CLASSE (Correção solicitada)
        # Ao usar average=None, ele retorna um array com o score de cada classe
        f_meas_per_class = fbeta_score(y_true_np, predicted_labels_mapped, beta=beta_val, average=None, zero_division=0)
        
        # Obtém as classes únicas ordenadas (o sklearn segue a ordem alfanumérica das classes)
        unique_classes = np.unique(y_true_np)
        
...
        # Vamos criar um DataFrame misto com as globais e as detalhadas
        
        # Lista com métricas globais
        # 7. Salvamento das Métricas
        summary_data = [
...
        ]
        
        # Adiciona as métricas por classe na lista
...
        
    # Este bloco calcula as métricas usando os GRAUS DE PERTINÊNCIA (Soft/Fuzzy)
    # conforme definido estritamente nas Equações 38 a 45 do artigo.
    # Não usamos rótulos 'hard' aqui, mas sim a matriz U contínua.

    print("\n--- Métricas Fuzzy Baseadas em Pares (Eqs. 38-45 com pertinência contínua) ---")
    
    try:
        # 1. Matriz de Coincidência Fuzzy (Psi_fuzzy) - Eq. 39
        # O elemento [j, k] é o produto escalar entre os vetores de pertinência de j e k.
        # Matematicamente: Psi_fuzzy = U . U_transposta
        Psi_fuzzy = best_U @ best_U.T  # Shape (N, N)

        # 2. Matriz de Coincidência Real (Psi_true) - Ground Truth
        # Para o Ground Truth, a pertinência é crisp (0 ou 1), então a coincidência também é.
        # O elemento [j, k] é 1.0 se j e k são da mesma classe, 0.0 caso contrário.
        y_np = y_true.values
        Psi_true = (y_np[:, None] == y_np[None, :]).astype(float)

        # 3. Extração dos Pares (Triângulo Superior)
        # As fórmulas 40-43 pedem o somatório para j > k (ou k > j).
        # Como as matrizes são simétricas e a diagonal não conta (j != k),
        # pegamos apenas os índices do triângulo superior (k=1).
        rows, cols = np.triu_indices(len(y_np), k=1)
        
        # Vetores contendo as coincidências para todos os pares únicos
        vec_fuzzy = Psi_fuzzy[rows, cols] # Valores entre 0 e 1
        vec_true = Psi_true[rows, cols]   # Valores 0.0 ou 1.0

        # 4. Cálculo dos Componentes da Tabela de Contingência (Eqs. 40-42)
        
        # N_SS (Eq. 40): Soma de (Psi_fuzzy * Psi_true)
        # Interpretação: O quanto de "força fuzzy" existe nos pares que SÃO reais.
        n_ss_fuzzy = np.sum(vec_fuzzy * vec_true)
        
        # N_SD (Eq. 41): Soma de (Psi_fuzzy * (1 - Psi_true))
        # Interpretação: O quanto de "força fuzzy" existe onde NÃO deveria haver relação.
        n_sd_fuzzy = np.sum(vec_fuzzy * (1 - vec_true))

        # N_DS (Eq. 42): Soma de ((1 - Psi_fuzzy) * Psi_true)
        # Interpretação: O quanto FALTOU de força fuzzy nos pares que SÃO reais.
...
        # Adiciona estas novas métricas fuzzy ao CSV
        fuzzy_metrics_data = [
...
        ]
        
        # Carrega dados atuais (se o dataframe metrics_df existir do passo anterior) ou cria novo
...
        print("\nArquivo final 'metricas_avaliacao_fuzzy_completas.csv' salvo com sucesso.")
...
    
    # Implementação baseada nas Equações 6 e 7 do artigo "Comparing Fuzzy Partitions"
    # Referência: Frigui et al. (citado na Seção III-A do texto).
    
    print("\n--- Fuzzy Rand Index (Frigui et al. - Eqs. 6 e 7) ---")
    
    try:
        # Nota: As variáveis 'vec_fuzzy' (psi_P) e 'vec_true' (psi_Q) já foram calculadas
        # no bloco anterior (Fuzzy Jaccard). Se você rodar este bloco isolado, 
        # precisará recalcular as linhas de 1 a 3 do bloco anterior.
        
        # Recuperamos os valores a, b, c calculados anteriormente
        # a = N_SS (Eq. 6a: Soma de psi_P * psi_Q)
        a_frigui = n_ss_fuzzy 
        
        # b = N_SD (Eq. 6b: Soma de psi_P * (1 - psi_Q))
        b_frigui = n_sd_fuzzy
        
        # c = N_DS (Eq. 6c: Soma de (1 - psi_P) * psi_Q)
        c_frigui = n_ds_fuzzy
        
        # CÁLCULO DE 'd' (N_DD) - O termo adicional do Rand Index
        # Eq. 6d: Soma de ((1 - psi_P) * (1 - psi_Q))
        # Representa pares que ambos concordam que DEVEM estar separados
        d_frigui = np.sum((1 - vec_fuzzy) * (1 - vec_true))
        
        # Denominador Total (Soma de todos os pares)
        total_pairs_frigui = a_frigui + b_frigui + c_frigui + d_frigui
        
        # Fuzzy Rand Index (Fórmula padrão aplicada aos componentes fuzzy)
...
        
        # --- Atualização do Arquivo de Métricas ---
        # Adiciona o Fuzzy Rand ao CSV existente
        
        new_metric_rand = [
...
            
        metrics_df_final_updated.to_csv("metricas_avaliacao_fuzzy_completas_v2.csv", index=False)
        print("\nArquivo final atualizado 'metricas_avaliacao_fuzzy_completas_v2.csv' salvo com sucesso.")
