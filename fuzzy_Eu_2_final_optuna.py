...
def local_gaussian_kernel_matrix(X, G, s_squared_matrix):
    """
    Calcula a matriz Kernel K(X, G) onde cada cluster (linha de G) usa o seu próprio vetor de larguras s_i.
    """
    N, D = X.shape
    ...
    for i in range(K_clusters):
        ...
        sum_sq = np.sum(weighted_diff_sq, axis=1) 
...
class KernelFCMWEU2:
    ...
    largura LOCAL (restrição de produto) e regularização de pertinência por Entropia (Tu).
    """
    ...
    def calculate_objective_function(self, X, G, U, s_squared):
        # Inércia Kernel Local (Linear em U, sem 'm')
        K_XG = local_gaussian_kernel_matrix(X, G, s_squared) 
        D_XG = 2 * (1 - K_XG)
        term1 = np.sum(U * D_XG)
        
        # Regularização por Entropia da Pertinência (T_U)
    ...
    def update_s_squared(self, X, G, U):
        ...
        for i in range(K_clusters):
            ...
            log_Q_i = np.log(Q_i + 1e-15) 
            numerator = np.exp(np.sum(log_Q_i) / D)
            
            new_inv_s_squared_matrix[i, :] = numerator / (Q_i + 1e-15)
    ...
    def fit(self, X, verbose=False):
        ...
        # Inicialização com prod(1/s_i^2) = 1 -> Inicializamos todos com 1.0
...
if __name__ == "__main__":
    ...
    if modo == '2':
        n_iteracoes = int(input("\nQuantas tentativas deseja que o Optuna faça? (Recomendado: 50 a 100): "))
        lim_inferior = float(input("Digite o limite inferior: "))
        lim_superior = float(input("Digite o limite superior: "))
        print(f"\nIniciando Otimização Bayesiana Paralelizada ({n_iteracoes} tentativas)...")

        # Função Objetivo unidimensional (somente Tu)
        def objective(trial):
            Tu_test = trial.suggest_float('Tu', lim_inferior, lim_superior, log=True)
            
            min_dists_rodadas = []
            for rep in range(n_repetitions_input):
                model_test = KernelFCMWEU2(n_clusters=n_clusters_input, Tu=Tu_test, tol=TOLERANCE, max_iter=max_iter_input, random_state=rep)
        ...
        print(f"Tentativas testadas: {n_iteracoes}")
    ...
    # --- Execução Principal do KFCM-K-W-EU.2 ---
    ...
    for i in range(n_repetitions_input):
        ...
        initial_prototypes, final_labels, final_U, final_G, final_s_squared, current_objective_value = kfcm_weu2.fit(X, verbose=False)
        ...
        #print(f"Valor final da função J_KFCM-K-W-EU.2: {current_objective_value:.6f}")
    ...
    df_G.to_csv("EU.2_prototipos_finais_otimos.csv", index_label="cluster")
    
    df_s_squared.to_csv("EU.2_s_squared_otimos.csv", index_label="cluster")
    
    pd.DataFrame(best_U).to_csv("EU.2_pertinencia_fuzzy_otima.csv", index=False)
    ...
    if y_true is not None:
        ...
        best_cm_df.to_csv("EU.2_matriz_de_confusao_otima.csv", index=True)
        
    df_results.to_csv("EU.2_resultados_clusters_otimos.csv", index=False)

    print("\nFicheiros de parâmetros ótimos (G, s^2, U, Resultados) salvos com sucesso.")
    ...
    if y_true is not None and len(all_metrics_history) > 0:
        ...
        stats_df.to_csv("EU.2_metricas_estatisticas_gerais.csv", index_label="Metrica")
        print("\nEstatísticas (Média, Desvio, Mediana, Máx, Mín) salvas em 'EU.2_metricas_estatisticas_gerais.csv'")

        print(f"\n--- Métricas da Melhor Execução (Rodada {best_round}) ---")
        best_metrics = df_all_metrics[df_all_metrics['Round'] == best_round].iloc[0]
        print(best_metrics)
        
        best_metrics.to_frame().T.to_csv("EU.2_metricas_melhor_rodada.csv", index=False)
        df_all_metrics.to_csv("EU.2_historico_metricas_todas_rodadas.csv", index=False)
